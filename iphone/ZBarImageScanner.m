...
#ifndef ZNO_DEBUG_CONSOLE
void zdlogf(const char *pattern, ...)
{
    //static char prevbuf[1000];
    //static int prevcount = 0;
    //static int duptimer = 0;

    char buf[1000];
    va_list ap;
    va_start(ap, pattern);

    if (vsnprintf(buf, sizeof(buf)-1, pattern, ap) >= sizeof(buf)) {
        sprintf(buf, "Message exceeds debug buffer (1000 chars)");
    }
...
- (NSInteger) scanImageRot: (ZBarImage*) image
{
    return(zbar_scan_image_rot(scanner, image.zbarImage));
}
