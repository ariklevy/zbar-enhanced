...
// scan the image at a given angle (in degrees)
// for efficiency, the coordinates are handled in a fixed-point
// format with 16 bits of fraction
void zbar_scan_image_pass (zbar_image_scanner_t *iscn,
                           zbar_image_t *img,
                           int angle, int scale, int band)
{
    const uint8_t *data, *p;
    zbar_scanner_t *scn = iscn->scn;
    unsigned w, h;
    long xmin, ymin, xmax, ymax, dx, dy, x, y;
    long stridedx, stridedy;
    int limit = 0;

    w = img->width;
    h = img->height;

    // calculate a 1-pixel step at the given angle
    dx = (1 << XYSHIFT) * cos(angle * M_PI / 180.0);
    dy = (1 << XYSHIFT) * sin(angle * M_PI / 180.0);

    // suck the boundaries in by 1 pixel so we don't have to do any checks
    // when interpolating
    xmin = (img->crop_x + 1) << XYSHIFT;
    ymin = (img->crop_y + 1) << XYSHIFT;
    xmax = (img->crop_x + img->crop_w - 1) << XYSHIFT;
    ymax = (img->crop_y + img->crop_h - 1) << XYSHIFT;

    // figure out where to start scanning by:
    //   (a) start at the middle of the image
    x = xmin + (xmax - xmin)/2;
    y = ymin + (ymax - ymin)/2;

    //   (b) calculate "stride", which walks us from one row to the next
    int density;
    if (band) {
        density = 5;
        stridedx = -dy * density;
        ...
        //   (c) scan "up" from the center to the topmost row.
        ...
        //       but not all the way off the image
        x += stridedx;
        y += stridedy;
    }

    //zdlogf( "stride = %x / %x", stridedx, stridedy );
    //x -= -dy * stride;
    //y -=  dx * stride;
    //   (d) scan to the start of this "row" within the boundaries
    dx = (dx * (scale>>2)) >> (XYSHIFT-2);
    dy = (dy * (scale>>2)) >> (XYSHIFT-2);
    while (x > xmin && x < xmax && y > ymin && y < ymax) {
        x -= dx;
        y -= dy;
    }
    ...
    for (;;) {
        ...
        //x = x&~0xffff;
        //y = y&~0xffff;
        // Point at the first pixel to inspect
        iscn->umin = virtu;
        //zdlogf( "base: %d, dir: %d", virtu, iscn->du );
        p = data + (x >> XYSHIFT) + (y >> XYSHIFT) * w;
        // scan while we're in bounds
        while (x > xmin && x < xmax && y > ymin && y < ymax) {
            long ox = x;
            long oy = y;
            // bilinear interpolation of pixel intensity
            uint8_t d = (
                  (p[0]   * ((((ox ^ -1) & 0xffff) * ((oy ^ -1) & 0xffff)) >> 16))
                + (p[1]   * ((((ox     ) & 0xffff) * ((oy ^ -1) & 0xffff)) >> 16))
                + (p[w]   * ((((ox ^ -1) & 0xffff) * ((oy     ) & 0xffff)) >> 16))
                + (p[w+1] * ((( ox       & 0xffff) * ( oy       & 0xffff)) >> 16))
              ) >> 16;
            //d = p[0];
            zbar_scan_y(scn, d);
            // step coords and pointer
            x += dx;
            y += dy;
            p += (x >> XYSHIFT) - (ox >> XYSHIFT);
            p += ((y >> XYSHIFT) - (oy >> XYSHIFT)) * w;
            ++samples;
            virtu += iscn->du;
        }
        // move to next row
        if (phase & 1) {
            ...
            y += stridedy;
            iscn->v += density;
            // if that put us back in bounds, keep scanning forward until we're out of bounds again
            while (x > xmin && x < xmax && y > ymin && y < ymax) {
                x += dx;
                y += dy;
                virtu += iscn->du;
            ...
            dx = -dx;
            dy = -dy;
            // scan back until we're in bounds again
            while (!(x > xmin && x < xmax && y > ymin && y < ymax)) {
                x += dx;
                y += dy;
                virtu -= iscn->du;
                // if we escaped the box, we're done
                if (   (x < xmin && dx < 0) || (x > xmax && dx > 0)
    ...
    //zdlogf("Samples: %d", samples);
}


int zbar_scan_image_rot (zbar_image_scanner_t *iscn,
                     zbar_image_t *img)
{
    zbar_symbol_set_t *syms;
    //const uint8_t *data;
    ...
    //zbar_image_t* useimg = zbar_image_adjust(img, NULL, 0.20, 0.2);
    zbar_image_t* useimg = zbar_image_unsharp_mask(img, 0.25, 4, 0.2);

    zbar_scan_image_pass(iscn, useimg, 0,   0x10000, 0);
    //zbar_scan_image_pass(iscn, img, 0,   0x11010);
    //zbar_scan_image_pass(iscn, useimg, 0,   0x15370);
    //zbar_scan_image_pass(iscn, img, 0,   0x0b0f0);
    zbar_scan_image_pass(iscn, useimg, 22,  0x10000, 0);
    zbar_scan_image_pass(iscn, useimg, 45,  0x10000, 0);
    zbar_scan_image_pass(iscn, useimg, 67,  0x10000, 0);
    zbar_scan_image_pass(iscn, useimg, 90,  0x10000, 0);
    zbar_scan_image_pass(iscn, useimg, 112, 0x10000, 0);
    zbar_scan_image_pass(iscn, useimg, 135, 0x10000, 0);
    zbar_scan_image_pass(iscn, useimg, 157, 0x10000, 0);

    zbar_scan_image_pass(iscn, useimg, 0,   0x10000, 1);
    zbar_scan_image_pass(iscn, useimg, 90,   0x10000, 1);

    zbar_image_destroy( useimg );
    ...
    int nean = 0, naddon = 0;
    if(syms->nsyms) {
        //zdlogf("Found: %d", syms->nsyms);
        zbar_symbol_t **symp;
        for(symp = &syms->head; *symp; ) {
            zbar_symbol_t *sym = *symp;
            //for (int i = 0; i < sym->npts; i++) {
            //    zdlogf( "Point: %d, %d", sym->pts[i].x, sym->pts[i].y );
            //}
